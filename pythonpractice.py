#!/usr/bin/env python3
'''
DATA 419 -- Assignment #0 support file
...
the_vars = {
    ...
    'Wheel': { 'Blue':['Moiraine','Siuan'], 'Green':['Alanna','Egweyne'],
        'Brown':['Verin','Saerin','Janya'], 'Red':['Elaida'],
        'Purple':[], 'White':['Sarene'], 'Yellow':['Nynaeve'] }
}
...
if all_present:
    if (Germanna_levels != UMW_levels and UMW_levels is MaryWash_levels and
        ...
        points += 1
        print("University variables correct! +1XP")
    else:
        print("University variables incomplete/incorrect.")
    

if 'plus2' in globals():
    if plus2(27) == 29:
        points += 1
        print("plus2() correct! +1XP")
    else:    
...
if 'gimme_dat_set' in globals():
    ...
    if thing1 is thing2 and thing1 == {'Navani','Shallan','Jasnah'}:
        points += 1
        print("gimme_dat_set() correct! +1XP")
    else:    
...
if 'gimme_set_like_dat' in globals():
    ...
    if (thing1 is not thing2 and thing1 == thing2 and
        thing1 == {'Kaladin','Dalinar','Adolin'}):
        points += 1
        print("gimme_set_like_dat() correct! +1XP")
    else:    
...
if 'wondrous_count' in globals():
    ...
    if all([wondrous_count_func(tv) == tw for tv,tw in test_vals.items()]):
        points += 1
        print("wondrous_count() correct! +1XP")
    else:    
